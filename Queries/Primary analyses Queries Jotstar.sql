...
select sum(total_watch_time_mins/60) as Avg_watch_time_in_hrs
from content_consumption;
...
WITH plan_changes AS (
    SELECT 
        ...
        CASE 
            WHEN subscription_plan = 'VIP' AND new_subscription_plan = 'Premium' THEN 'VIP to Premium'
            WHEN subscription_plan = 'VIP' AND new_subscription_plan = 'Free' THEN 'VIP to Free'
...
WITH plan_changes AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN subscription_plan = 'Free' AND new_subscription_plan = 'VIP' THEN 'Free to VIP'
            WHEN subscription_plan = 'Premium' AND new_subscription_plan = 'VIP' THEN 'Premium to VIP'
            ELSE NULL 
        END AS upgrade_type
    FROM subscribers
    WHERE plan_change_date IS NOT NULL
...
WITH user_months AS (
    SELECT 
        ...
        CASE 
            WHEN s.subscription_plan = 'VIP' THEN 169
            WHEN s.subscription_plan = 'Premium' THEN 359
